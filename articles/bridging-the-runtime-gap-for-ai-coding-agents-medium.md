...
**You encounter a bug** → You open Chrome DevTools (F12) → You manually copy the error message & stack trace → You inspect the Network tab, copy the request payload & 422 error response → You take a screenshot of the broken UI → You switch to your IDE and write a 4-paragraph prompt:

> *"I was logged in as an admin, clicked on the checkout coupon button, typed 'SAVE20', submitted the form, and the button got disabled but nothing updated. Here is the console error, here is the network payload…"*
...
**① A Manifest V3 Chrome Extension** — Attaches to your active browser tab using the Chrome DevTools Protocol (CDP) and a non-intrusive background tracker. It observes exceptions, network requests, user interactions (breadcrumbs), and internal framework state.

**② A Local MCP Server** — Runs locally alongside your IDE (Cursor, Claude Code, etc.), exposing high-level diagnostic tools that AI models can invoke on demand.

**How the pieces connect:**

Your **Google Chrome** browser — with your authenticated tab (SSO, 2FA, cookies all intact) — runs the **DOMRay Extension**, which captures telemetry via the Chrome DevTools Protocol (CDP). This extension communicates over a secure local WebSocket (`ws://127.0.0.1`) — protected by token authentication and CSWSH validation — with the **DOMRay MCP Server** running alongside your IDE. The MCP server then exposes this runtime data to your **AI Coding Assistant** (Cursor, Claude Code, etc.) via the standard MCP stdio protocol.

**Everything stays on your machine. Zero data leaves your workstation.**
...
* **Detect React 18/19 Containers:** Automatically recognizes `__reactContainer$` on `#root` or `body` and navigates down to user components (e.g. your App root component).
...
* **Support Vue 3 in Production:** Inspects `_vnode.component` and unwraps reactive ref values (`_v_isRef`) inside `setupState`.

### 4. Semantic DOM Sanitizer (-80% Token Reduction)
Passing raw HTML to an LLM is wasteful. DOMRay features a token-pruning engine:
* Replaces massive SVG icons with concise placeholders containing only the aria-label and role attributes.
* Strips script, style, link, and template tags entirely.
...
* DOMRay navigates the React Fiber tree up to the CouponForm component.
...
```json
{
  ...
  "state": [
    ...
    { "hook": 1, "type": "useState", "value": true }
  ]
}
```

☝️ `hook 1` is `isSubmitting` — stuck at `true`!
